	  		...
	  		<div className="flex flex-row admin-quest-container gap-3">
		  		<div className="quest-left-containter">
					...
					<div className='quest-reward-card'>
							<p className='main-title'>3. Type of reward</p>
							<RewardTypeElement title="Karma" checked={null} onToggle={()=>console.log("type of reward : karma")}/>
							{
								items.map((item , index) => (
									<RewardTypeElement key={index} title={item.title} checked={checked === index} onToggle={() => handleToggle(index)}/>
								))
							}
					</div>
		  		</div>
